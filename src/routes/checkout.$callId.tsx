      ...
      const checkout = new Razorpay({
        ...
        handler: async (response: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          try {
            ...
            // Wait for the verified paid sheet to load before navigating.
            // This prevents the customer from briefly (or incorrectly) seeing
            // the locked teaser after a successful payment.
        ...
        <aside className="space-y-5 lg:sticky lg:top-24">
          <section className="card-premium rounded-3xl p-6">
            ...
            <p className="mt-3 flex items-center justify-center gap-1.5 text-center text-[10px] text-muted-foreground">
              <ShieldCheck className="h-3 w-3 shrink-0" /> Secure payment · instant access
            </p>
