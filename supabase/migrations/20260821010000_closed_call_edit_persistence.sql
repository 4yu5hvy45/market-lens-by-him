-- Market Lens: make closed-call realised P&L overrides available to the public projection.
-- Also makes this migration safe to run even if the earlier override migration was not applied yet.
...
SELECT
  ...
  CASE WHEN c.state = 'live' THEN NULL ELSE c.realised_pnl_pct_override END AS realised_pnl_pct_override,
