-- Allow the desk to explicitly control the public "Potential left" figure.
-- NULL means: calculate it from entry/target as before.
...
SELECT
  ...
  COALESCE(
    ...
    ROUND(((c.target - c.entry) / c.entry) * 100 * CASE WHEN c.direction = 'short' THEN -1 ELSE 1 END, 2)
  ) AS potential_pct,
